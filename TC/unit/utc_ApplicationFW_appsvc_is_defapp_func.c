...
static void utc_appsvc_is_defapp_func_01(void)
{
	int ret = 0;
	ret =
	    appsvc_set_defapp("http://tizen.org/appcontrol/operation/abcd123", NULL, NULL,
			      "com.samsung.abcd123");
	...
	ret = appsvc_is_defapp("com.samsung.abcd123");
	if (ret == 1) {
	...
	appsvc_unset_defapp("com.samsung.abcd123");
}
...
{
	int ret = 0;
	ret = appsvc_is_defapp("com.samsung.abcdefgh");
	if (ret == 0) {
		tet_infoline("appsvc_is_defapp test PASS!");
		tet_printf("ret = %d", ret);
		tet_result(TET_PASS);
